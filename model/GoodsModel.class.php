...
class GoodsModel extends Model {
	...
	public function getCompare(){
		if(isset($_COOKIE['compare'])){
			...
			$_compareGoods = parent::select(array('id','nav','name','(select name from mall_brand c where c.id=a.brand) as brand','sn','attr','weight','price_sale','price_market','thumbnail','thumbnail2','unit','sales','(select count(*) from mall_commend b where flag=0 and b.goods_id=a.id) as count'),
										array('where'=>array("id in ($_compareStr)")));
			$this->_tables = array(DB_FREFIX.'goods');
			//print_r($_compareGoods);
	...
	public function collectGoods(){
		...
		$_collectGoods = parent::select(array('id','nav','name','price_sale','price_market','thumbnail','thumbnail2','unit','sales','(select count(*) from mall_commend b where flag=0 and b.goods_id=a.id) as count'),
										array('limit'=>$this->_limit,'where'=>array("id in ($_collectStr)"),'order'=>'date DESC'));
	...
	public function salesGoods(){
		$this->_tables = array(DB_FREFIX.'goods a');
		$_salesGoods = parent::select(array('id','nav','name','price_sale','price_market','thumbnail2','sales','(select count(*) from mall_commend b where flag=0 and b.goods_id=a.id) as count'),
	...
	public function allGoodsCount(){
		$allGoodsCount = parent::select(array('COUNT(*) as count'));
		return $allGoodsCount[0]->count;
	}

	public function inventoryGoodsCount(){
		$inventoryGoodsCount = parent::select(array('COUNT(*) as count'),
												array('where'=>array("inventory<=warn_inventory")));
		return $inventoryGoodsCount[0]->count;
	}
	...
	public function setInventory(){
		...
		foreach($_goods as $_key=>$_value){
			...
			$_updateData = array('inventory'=>array('inventory-'.$_value->num),'sales'=>array('sales+'.$_value->num));
