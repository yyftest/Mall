...
class DeliveryAction extends Action{
	...
	public function add(){
		if(isset($_POST['send'])) $this->_model->add()?$this->_redirect->succ('?a=delivery','添加快递成功！'):$this->_redirect->error('添加快递失败！');
		$this->_tpl->display(SMARTY_ADMIN.'delivery/add.tpl');
	}

	public function update(){
		if(isset($_POST['send'])) $this->_model->update()? $this->_redirect->succ('?a=delivery', '快递修改成功！') : $this->_redirect->error('快递修改失败！');
		if(isset($_GET['id'])){
