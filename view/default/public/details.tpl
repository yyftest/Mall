...
<div id="main">
	...
	<dl id="text" class="text">
		...
		{/if}
		<a href="?a=member&m=addCollect&id={$FrontGoods[0]->id}" target="_blank"><img src="view/default/images/collect.gif" alt="收藏" title="收藏" /></a></dd>
	</dl>
	</form>
	<div class="content">
		<a name="commend"></a>
		...
		<div class="c3" id="c3" style="display:none;">
			<table border="0" cellspacing="0" width="100%" class="record">
				...
				<tr><td>{$value->user}</td><td style="text-align:center;line-height:180%;" title="{$value->name}">{$value->name|truncate:50}<br /><span style="color:#666;">{$value->attr}</span></td><td>{$value->price}</td><td>{$value->num}</td><td>{$value->date}</td></tr>
				{/foreach}
			</table>
			<div id="page">{$page}</div>
		</div>
