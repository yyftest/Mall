...
<table id="cart" cellspacing="1">
	...
	</td><td class="weight">{$value.weight}/kg</td><td class="price">{$value.price_sale}/元</td><td><input type="text" name="num" onblur="changeNum({$value.id},this.value);" class="small" value="{$value.num}" /></td><td class="price">{$value.price_sale*$value.num}/元</td><td><a href="?a=cart&m=delProduct&id={$value.id}">删除</a></td></tr>
	{/foreach}
</table>
